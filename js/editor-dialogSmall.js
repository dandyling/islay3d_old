...
var DialogRenameCharacter = function(charPanel){
	...
	if(stage.get("#layerCharRenameDialog")[0] == undefined){
		var layer = new Kinetic.Layer({id: "layerCharRenameDialog"});
		stage.add(layer);
		stage.arrangeLayer();
	}
	else {
...
var DialogLogin = function(){
	//dialogBoxes.close();
	dialogBoxes.closeSmallDialogs();
	var layer = new Kinetic.Layer({id: 'layerLogin'});
	stage.add(layer);
	stage.arrangeLayer();
...
var DialogRenameDiagram = function(index){
	dialogBoxes.close();
	var layer = new Kinetic.Layer({id: "layerDiagRemoveDialog"});
	stage.add(layer);
	stage.arrangeLayer();
	...
	var textField = new TextField(
			"inputTextNewDialogName",
			stage.selectedPanel.selectedDiagram.label,
			5,
			25);
	$(textField).offset({
		left : $(inputDivParent).offset().left + $(inputDivParent).width()/2 - $(textField).width()/2 
...
var DialogDuplicateCharacter = function(tabs){
	dialogBoxes.close();
	var layer = new Kinetic.Layer({id: "layerDiagDuplicateChar"});
	stage.add(layer);
	stage.arrangeLayer();
